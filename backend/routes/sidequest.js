...
// ⚠️ Must be before any /:id routes
router.get("/notifications", async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(`
      SELECT
        n.id, n.type, n.sidequest_id, n.is_read, n.created_at,
        s.event_title, s.event_des, s.location, s.time_of_event, s.circle_status,
    ...
    console.error("[GET /events/notifications]", err);
    res.status(500).json({ error: err.message });
  }
});

// ─── PATCH /events/notifications/:id/delete ───────────────────────────────────
router.patch("/notifications/:id/delete", async (req, res) => {
  const { id } = req.params;
  try {
    await pool.query(`UPDATE notifications SET is_deleted = true WHERE id = $1`, [id]);
    res.json({ success: true });
  } catch (err) {
    console.error("[PATCH /notifications/:id/delete]", err);
    res.status(500).json({ error: err.message });
  }
});

// ─── PATCH /events/notifications/:id/read ────────────────────────────────────
...
router.post("/", async (req, res) => {
  const {
    user_id, event_title, event_des, location,
    time_of_event, time_event_end, max_attendees, circle_status,
  } = req.body;
  ...
  try {
    ...
    const { rows } = await pool.query(`
      ...
      RETURNING *
    `, [newId, user_id, event_title, event_des ?? "", location ?? "TBD",
        time_of_event, time_event_end, max_attendees ?? null, circle_status]);

    const { rows: full } = await pool.query(`
      SELECT s.*, u.first_name AS poster_first_name, u.last_name AS poster_last_name, u.location AS poster_location
      FROM sidequests s
      LEFT JOIN users u ON u.id = s.user_id
      WHERE s.id = $1
    `, [rows[0].id]);
...
router.post("/:id/join", async (req, res) => {
  ...
  try {
    ...
    const lockResult = await client.query(
      `SELECT id, user_id, max_attendees FROM sidequests WHERE id = $1 FOR UPDATE`,
      [id]
    );
    ...
    if (countResult.rows[0].count >= (sidequest.max_attendees ?? 1)) {
      await client.query("ROLLBACK");
    ...
    const { rows: full } = await pool.query(`
      SELECT s.*, u.first_name AS poster_first_name, u.last_name AS poster_last_name, u.location AS poster_location
      FROM sidequests s LEFT JOIN users u ON u.id = s.user_id
      WHERE s.id = $1
    `, [id]);
...
router.patch("/:id", async (req, res) => {
  const { id } = req.params;
  const { user_id } = req.query;
  const { event_title, event_des, location, time_of_event, time_event_end, max_attendees, circle_status } = req.body;
  ...
  try {
    ...
    `, [event_title, event_des, location, time_of_event, time_event_end, max_attendees, circle_status, id, user_id]);
